...
public class MapGenerator : MonoBehaviour
{
	...
	{
		return (_coord((int)coord.y, (int)coord.x));
	}
	...
	GameObject BoardSetup ()
	{
		//Instantiate Board and set boardHolder to its transform.
		GameObject retval = new GameObject("Board");
		boardHolder = retval.transform;
		for (int x = 0; x < columns + 1; x++)
		{
			for (int y = 0; y < rows + 1; y++)
			{
	...
	private void PopRiver(Vector2 coord, Vector2 last, Vector2 direction)
	{
		if (coord.x == 0 || coord.y == 0 || coord.x == columns || coord.y == rows || grid[_coord(coord)].ftype == FloorType.WATER)
			return;
		float riverRange = 3F;
		Vector2 orth = new Vector2(-(coord.y - last.y), coord.x - last.x);
		grid[_coord(coord)].ftype = FloorType.WATER;
		grid[_coord(coord)].height /= 2.0f;
		if (orth.x != 0)
		{
			InterpolateLineX(coord + riverRange * orth, coord);
			InterpolateLineX(coord - riverRange * orth, coord);
		}
		else
		{
			InterpolateLineY(coord + riverRange * orth, coord);
			InterpolateLineY(coord - riverRange * orth, coord);
		}
	...
	private void PlasmaHeightField(Vector2 a, Vector2 b, Vector2 c, Vector2 d)
	{
		...
		if (b.x - d.x > 2)
		{
			...
			float variance = ((b.x - d.x) / columns) * 1f;
	...
	private GameObject GenerateBoard(int width, int height)
	{
		...
		int tmprows = rows;
		FieldInfo[] tmpgrid = grid;
		Transform tmpbh = boardHolder;
	...
	public GameObject GenerateOverworld()
	{
		//Version temporaire en dur
		columns = 10;
		rows = 10;
		InitMapGrid();
		for (int x = 0; x < columns; x++)
	...
	{
		maxHeight = 5F;
		GameObject overworld = GenerateOverworld();
